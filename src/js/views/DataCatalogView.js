...
define(['jquery',
 ...
 'click .filter-contain .expand-collapse-control' : 'toggleFilterCollapse',
		...
		render: function () {
			...
			// Adding JSONLD to the DataCatalog page if it is enabled.
		 ...
		 *
		 * @param {object} elJSON - JSON-LD to insert into the page
		 *
		 * Some notes:
		 *
		...
		insertJSONLD: function(elJSON) {
		 ...
		 /**
		 * Generate Schema.org-compliant JSONLD for the DataCatalog model bound to the view into
		...
		generateJSONLD: function () {
			...
			let elJSON = {
				...
				"contactPoint" : {
					"@id": "https://www.dataone.org/contact",
					"@type": "ContactPoint",
					"name": "Support",
					"email" : "",
			...
			if(nodeModelObject) {
				...
				if (nodeModelObject.description) {
					elJSON["description"] = nodeModelObject.description
				}

				if (nodeModelObject.identifier) {
					elJSON["identifier"] = nodeModelObject.identifier
				}

				if (nodeModelObject.name) {
					elJSON["name"] = nodeModelObject.name
				}

				if (nodeModelObject.url) {
					elJSON["url"] = nodeModelObject.url
				}


				if (nodeModelObject.logo) {
					elJSON["logo"] = nodeModelObject.logos
				}
			}
			else {
				// If the 
				$.ajax({
					url: window.location.origin + "/metacat/d1/mn/v2/node",
					dataType: "text",
					success: function(data, textStatus, xhr) { 
						...
						if(!xmlResponse) return;
							...
							if ($(xmlResponse).find("identifier")) {
								elJSON["identifier"] = $(xmlResponse).find("identifier").text()
							}

							if ($(xmlResponse).find("name")) {
								elJSON["name"] = $(xmlResponse).find("name").text()
							}

							if ($(xmlResponse).find("baseURL")) {
								elJSON["url"] = $(xmlResponse).find("baseURL").text()
							}

						}
						
						view.insertJSONLD(elJSON);
					}
				});
			}
